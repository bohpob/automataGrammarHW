...
DFA initializeDFA(const set<Symbol> &nfaAlphabet) {
    DFA dfa;
    dfa.m_States.insert(0);
    dfa.m_InitialState = 0;
    dfa.m_Alphabet = nfaAlphabet;
    return dfa;
}

set<State> getNextStates(const MISNFA &nfa, const set<State> &states, const Symbol &symbol) {
    set<State> result;
    for (const auto &state: states) {
        const auto &transition = nfa.m_Transitions.find({state, symbol});
...
bool convertNFAToDFA(DFA &dfa, const MISNFA &nfa) {
    map<set<State>, State> stateMap;
    set<State> initialState = nfa.m_InitialStates;
    ...
    while (!stateQueue.empty()) {
        ...
        for (const auto &symbol: nfa.m_Alphabet) {
            set<State> nextStates = getNextStates(nfa, currentState, symbol);

            if (nextStates.empty())
                continue;

            if (stateMap.find(nextStates) == stateMap.end()) {
                State newState = dfa.m_States.size();
                dfa.m_States.insert(newState);
                stateQueue.push(nextStates);
                stateMap[nextStates] = newState;
            }

            dfa.m_Transitions[{stateMap[currentState], symbol}] = stateMap[nextStates];
        }
    }

    set<State> finalStates;
    for (const auto &i: stateMap)
        for (const auto &y: i.first)
            for (const auto &finalState: nfa.m_FinalStates)
                if (y == finalState)
                    finalStates.insert(i.second);
...
void removeUselessStates(DFA &dfa) {
    set<State> usefulStates = dfa.m_FinalStates;
    queue<State> finalStates;
    for (const auto &state: dfa.m_FinalStates)
        finalStates.push(state);

    while (!finalStates.empty()) {
        State currentState = finalStates.front();
        finalStates.pop();

        for (const auto &transition: dfa.m_Transitions) {
            if (transition.second == currentState) {
                State prevState = transition.first.first;
                if (usefulStates.find(prevState) == usefulStates.end()) {
                    usefulStates.insert(prevState);
                    finalStates.push(prevState);
                }
            }
        }
    }
    ...
    set<State> newStates;
    for (const auto &state: dfa.m_States)
        if (usefulStates.find(state) != usefulStates.end())
            newStates.insert(state);

    map<pair<State, Symbol>, State> newTransitions;
    for (const auto &transition: dfa.m_Transitions) {
        if (newStates.find(transition.first.first) != newStates.end() &&
            newStates.find(transition.second) != newStates.end()) {
    ...
    for (const auto &finalState: dfa.m_FinalStates) {
        if (newStates.find(finalState) != newStates.end())
            newFinalStates.insert(finalState);
    }
    dfa.m_FinalStates = newFinalStates;

    dfa.m_States = newStates;
}

DFA determinize(const MISNFA &nfa) {
    DFA dfa = initializeDFA(nfa.m_Alphabet);

    if (!convertNFAToDFA(dfa, nfa)) {
        dfa.m_States = {0};
        dfa.m_FinalStates = {};
        dfa.m_InitialState = 0;
        dfa.m_Transitions = {};
